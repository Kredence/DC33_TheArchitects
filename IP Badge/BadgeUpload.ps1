...
mpremote reset
Write-Host "Issuing a board reset for boot.py to run and sleep 15..."
Start-Sleep -Seconds 15

Write-Host "`n--- Step 3: Copy external folder ---"

$maxAttempts = 5

for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
    Write-Host "Attempt $($attempt): Copying .\external\. to :\external..."

    mpremote cp -r .\external\. :external
    $exitCode = $LASTEXITCODE

    if ($exitCode -eq 0) {
        Write-Host "External folder copied successfully!"
        break
    } else {
        Write-Host "Copy failed with exit code $exitCode."
